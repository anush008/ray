...
@contextmanager
def _continuous_http_traffic(expected_responses):
    """Send requests until the caller finishes and fail on any request error."""
    stop = threading.Event()
    started = threading.Event()
    responses = []
    errors = []
        ...
        with httpx.Client(timeout=10) as http:
            while not stop.is_set():
                try:
                    response = http.get("http://localhost:8000/")
                    assert response.status_code == 200, response.text
                    assert response.text in expected_responses, response.text
                    responses.append(response.text)
                except Exception as exc:
                    errors.append(repr(exc))
                finally:
...
def test_sparse_config_rollback_restores_code_defined_options(
    ...
    with _continuous_http_traffic({"ok"}) as responses:
        ...
        wait_for_condition(lambda: len(_running_replica_pids(client)) == 2, timeout=120)
